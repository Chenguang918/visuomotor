% Practice for Free time NeuroFeedback Visual search 
...
%     end        
    %% 参数设置部分
    % ------submessage-------
    prompt={'Subid','Name','Order'};
    dlg_title='submessage';
    num_lines=1;
    defaultanswer={'01','Test','1'};
    subinfo=inputdlg(prompt,dlg_title,num_lines,defaultanswer);
    commandwindow;
    % dataPath='E:\hyq\NFB\';
     dataPath='C:\Users\15455\Documents\MyWork\NFB\Data\';
    %%
    subid_name_file=strcat('sub',subinfo{1},subinfo{2});
    filepath=strcat(dataPath,'\experimentdata\subexpdata\','sub',subinfo{1},subinfo{3},'_practice','\');
    if ~exist(filepath,'dir')
         mkdir(filepath);
    end
    
     % filename=strcat(filepath,subid_name_file); % 这一步没有用！
...
%% Introduction
sttext = 'Press SPACE key to start Practice';
...
% 不同刺激条件分配不同比例
% Nt
% condition_index(list(1:5))=11;    % 10 T上 
%    
% 
% % 不同刺激条件分配不同比例
% % Nt
condition_index(list(1:10))=11;    % 10 T上 
condition_index(list(11:20))=21;  % 10 T  
% Pd
condition_index(list(21:30))=12; % 10 D上 
condition_index(list(31:40))=22; % 10      
% Opposite 
condition_index(list(41:42))=13; % 2 T左D右
condition_index(list(43:45))=23; % 3 T右D左 
% Samesite
condition_index(list(46:47))=14; % 2 同左   
% condition_index(list(48:50))=24; % 3 同右   
...
for ntrial=1:trial
    ...
    switch ma        
        ...
        case 2 % cue:red&green  tar midline  _Pd
            switch arrowindex
                case 1 % D:left T:up or down
                   disloca=randi([6,9],1,1);
                   tarloca=5*randi([1,2],1,1); 
                case 2 % D:right T:up or down
...
load(filename);
ACC = mean(fidnew.ACC);
RightIn = fidnew.ACC == 1; %ACC=1的index,以防漏按
ACC = mean(RightIn,2)*100; %求正确率
Result=[ 'ACC=',num2str(ACC),'%'];

Screen('FillRect', w, background);
Screen('DrawText', w , Result, 4*center(1)/5, center(2), gra);
Screen('Flip', w);
WaitSecs(firstfixtime);  
