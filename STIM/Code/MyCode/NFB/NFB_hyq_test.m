% Free time NeuroFeedback Visual search with Behaviour Results automaticly
% being calculated.
...
%     dataPath='E:\Zhaochenguang\NFB\';
    dataPath='C:\Users\15455\Documents\MyWork\Data\NF\';
...
%To calculate the coordinates of the center of every circle
...
%% cue&target array
dislocindex=[7,8,9;6,8,9;6,7,9;6,7,8]; % 为了后面编写刺激所在的位置
...
%pre-stimulus (1 s) 
    while 1
        ...
        toc % 
        if power==protocol(blockindex)  % 监测是否满足协议
            break
        end
    end   
%     tic  
...
%     Total_Power(ntrial)=power;
   ...
   % 呈现刺激
    ...
    WaitSecs(0.004); % 等0.004s？
    %outp(888, 0);
    WaitSecs(stimtime); % 刺激呈现200ms
    ...
    while toc < blanktime
        ...
        if touch && (keyCode(rightkey) || keyCode(wrongkey)) % 按照指令按键
     ...
     if keyCode(rightkey) % 按键正确
        ACC(blockindex,ntrial) = 1;
    elseif keyCode(wrongkey) %按键错误
        ACC(blockindex,ntrial) = 0;
    elseif keyCode(EscapeKey) % 终断程序
        break;
        Screen('CloseAll');
     end
        %outp(888, 99+ACC(blockindex,ntrial)); % 正确marker
        WaitSecs(0.004);
        %outp(888, 0);
    LO(blockindex,ntrial) = Tlineindex; % line朝向
    ArrT(blockindex,ntrial) = arrowindex; 
end
Screen('DrawText', w, breaktext, 7*center(1)/10, center(2), gra, background);
Screen('Flip', w);
  
KbWait;%等待直到按键，开始下一个block
end


Screen('FillRect', w, background);
Screen('DrawText', w , endtext, 4*center(1)/5, center(2), gra);
Screen('Flip', w);
WaitSecs(firstfixtime);
...
%% behavior Result 
[ACC,RT,RTsd,RTmid]=RunBehavior4NFB(filename);
