...
path_out_eeg = 'E:\Data\202305\EEG\8.seg_env\';
...
function EEG_BLK = get_eeg_env(NBLK, BAND_ALPHA, BAND_ALL, EEG_BLK)
    for iblk = 1:NBLK
        EEG = EEG_BLK(iblk);
        mEEG = pop_eegfiltnew(EEG, BAND_ALPHA(1), BAND_ALPHA(2));
        mEEG_data = abs(hilbert(mEEG.data'))';
%         EEG = pop_eegfiltnew(mEEG, BAND_ALL(1), BAND_ALL(2));
%         EEG = eeg_checkset(EEG);
%         EEG_data = abs(hilbert(EEG.data'))';
        EEG_BLK(iblk).data = mEEG_data;
    end
end
